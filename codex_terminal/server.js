...
let state={threadId:null,messages:[],activities:[],models:[],selectedModel:null,authenticated:false,login:null,permissionMode:process.env.PERMISSION_MODE||'default',workingDirectory:process.env.WORKING_DIRECTORY||'/config'};
...
const appServerArgs=(state.permissionMode==='full-access'?['--dangerously-bypass-approvals-and-sandbox']:[]).concat(['app-server','--listen','stdio://']); const codex=spawn('codex',appServerArgs,{env:{...process.env,HOME:DATA,CODEX_HOME:path.join(DATA,'.codex')},stdio:['pipe','pipe','inherit']});
...
wss.on('connection',ws=>{ws.send(JSON.stringify({type:'state',state}));ws.on('message',async raw=>{let m;try{m=JSON.parse(raw)}catch{return}try{
 if(m.type==='send'){if(!state.authenticated)throw new Error('Codex n’est pas authentifié');await send(m.text||'',m.attachments||[]);} else if(m.type==='login'){const lr=await rpc('account/login/start',{type:'chatgptDeviceCode'});state.login={loginId:lr.loginId,verificationUrl:lr.verificationUrl,userCode:lr.userCode};save();broadcast({type:'state',state});}
