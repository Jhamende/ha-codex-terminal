...
let state={threadId:null,messages:[],permissionMode:process.env.PERMISSION_MODE||'default',workingDirectory:process.env.WORKING_DIRECTORY||'/config'};
...
codex.stdout.on('data',d=>{buf+=d;let i;while((i=buf.indexOf('\n'))>=0){const line=buf.slice(0,i);buf=buf.slice(i+1);if(!line.trim())continue;let m;try{m=JSON.parse(line)}catch{continue}
 ...
 if(method==='item/agentMessage/delta')broadcast({type:'delta',delta:p.delta||'',itemId:p.itemId});
...
wss.on('connection',ws=>{ws.send(JSON.stringify({type:'state',state}));ws.on('message',async raw=>{let m;try{m=JSON.parse(raw)}catch{return}try{
 ...
 else if(m.type==='newThread'){state.threadId=null;state.messages=[];save();broadcast({type:'state',state})}
